"""
train_temporal_poc_ucolaeo.py — UCO-LAEO port of train_temporal_poc_childplay.py.

WHAT'S IDENTICAL, COPIED VERBATIM: every model class (GRUHead,
TransformerHead, ResidualGRUHead, ResidualTransformerHead,
EmbeddingZeroedWrapper), make_regression_tail/make_residual_tail,
count_params, pick_nhead, seed_everything, evaluate(), train_one_model()
— all of this operates purely on cached embedding tensors and is
completely dataset-agnostic. None of it changes here either.

compute_baseline_computable_mask() ALSO needed zero logic changes, not
just a rename — confirmed by checking field-by-field:
temporal_window_dataset_ucolaeo.py's TemporalWindowDataset.rows carries
"clip", "subject", "last_context_path" under the exact same keys
ChildPlay's version does, and
compute_last_known_position_baseline_ucolaeo.py's get_subject_gaze/
parse_frame_number were deliberately built to match ChildPlay's
signatures exactly. Only the import line changed.

WHAT ACTUALLY NEEDED ADAPTING:
1. Imports: temporal_window_dataset_ucolaeo /
   compute_last_known_position_baseline_ucolaeo instead of the ChildPlay
   versions.
2. NO GAZE-TYPE BREAKDOWN TO PORT: UCO-LAEO has no object category at
   all (100% social by construction) — but this required no code
   removal, because train_temporal_poc_childplay.py's own main() never
   computed a gaze-type breakdown in the first place (that analysis
   lives in train_temporal_poc_childplay_balanced.py and separate ad hoc
   work, not this file). This script's own summary was already pooled-
   only. target_gaze_type is still carried through __getitem__ and the
   batch dict, unused here, harmless — every value will just be
   "social".
3. NO ORACLE-BASELINE TABLE CROSS-REFERENCE: the ChildPlay version's
   closing text points at compute_last_known_position_baseline_childplay.py's
   own "VAL-SPLIT ONLY" table for the oracle ADE comparison — that
   broader table-printing feature was never built for UCO-LAEO
   (compute_last_known_position_baseline_ucolaeo.py is deliberately a
   minimal lookup helper, not a full report generator; see that file's
   own docstring). Flagged as a real gap below rather than referencing
   something that doesn't exist.

A REAL BUG FIXED DURING THIS PORT, NOT CARRIED FORWARD: the ChildPlay
version's own docstring and runtime print both claim "LR defaults to
1e-3, matching VAT's original starting point" — this was already flagged
elsewhere (2026-07-30 session notes) as stale and wrong; the actual code
there is `LR = float(os.environ.get("POC_LR", 1e-5))`, confirmed by
reading the file directly. That mismatch was never corrected in the
source. Rather than copy the same lie into a new file, LR here defaults
to 1e-5 (VAT's actual established value, and what ChildPlay's code
really ran at regardless of what it claimed) with accurate documentation
— POC_LR is still available as an override if a deliberate 1e-3
starting-point replication is ever wanted.

ADDED AFTER THE FIRST REAL RUN (2026-07-31): paired bootstrap significance
testing (10,000 resamples), same methodology ChildPlay's own investigation
used to confirm its effects were robust rather than noise. Added here
because this run's val set is small (398 rows, ~1/17th of ChildPlay's),
which is exactly the regime where a headline margin is most likely to be
an artifact of which particular rows landed in the split. Computed inline,
directly from each model's already-in-memory per-row distances, rather
than as a separate script needing saved per-row data -- avoids any
possibility of the significance numbers drifting out of sync with a
...
from temporal_window_dataset_ucolaeo import (
...
# either model is constructed, same as VAT/ChildPlay.

EPOCHS = int(os.environ.get("POC_EPOCHS", 180))
BATCH_SIZE = int(os.environ.get("POC_BATCH_SIZE", 64))
LR = float(os.environ.get("POC_LR", 1e-5))  # see module docstring -- VAT's
                                             # actual established value,
...
def main():
    ...
    print(f"Horizon scope: target_horizon_ms={PRIMARY_HORIZON_MS} only, matching the VAT/ChildPlay PoC's own scope.")
    print("Dataset note: UCO-LAEO is 100% social by construction (no object-gaze "
          "category exists) -- every target_gaze_type below will read 'social'. "
          "This PoC therefore tests the POOLED replication question only, not "
          "the object/social breakdown VAT and ChildPlay could ask.\n")
    ...
    if input_dim > 2:
        ...
        print("  THIS is the fair floor a temporal model needs to beat. Compare "
              "against domain_transfer_check_ucolaeo.py's own zero-horizon "
    ...
    # ── Summary ──────────────────────────────────────────────────────
    print("\n" + "=" * 78)
    print("SUMMARY (UCO-LAEO, pooled -- no object/social breakdown exists for this dataset)")
    print("=" * 78)
    ...
    print("\nNo oracle-ADE table exists for UCO-LAEO yet (unlike VAT/ChildPlay, "
          "whose baseline scripts print a full 'VAL-SPLIT ONLY' oracle table -- "
          "compute_last_known_position_baseline_ucolaeo.py is a minimal lookup "
          "helper, not a report generator; see that file's own docstring). "
    ...
    print("A margin is only reported ROBUST if its 95% CI stays entirely "
          ...
          "set, not just in the one actual sample observed. Small n (398 "
          "here, vs ChildPlay's thousands) makes this check matter more, "
          "not less.\n")
