...
def load_vacation_fps(fps_source_path):
    """NOT YET IMPLEMENTED -- no confirmed VACATION fps source file was
    available when this script was written (vacation_temporal_utils.py
    expects 'video_fps.txt', format: '{video_id}.mp4: {fps} FPS' per
    line, but that real file wasn't provided). Wire this in once it's
    available, matching vacation_temporal_utils.load_fps_map()'s own
    parsing exactly -- don't guess a different format."""
    raise NotImplementedError(
        "VACATION fps source not yet wired in -- see module docstring. "
        "Provide the real video_fps.txt (or equivalent) and implement "
        "this function to match vacation_temporal_utils.load_fps_map()'s "
        "own parsing before running vacation through this script."
    )
