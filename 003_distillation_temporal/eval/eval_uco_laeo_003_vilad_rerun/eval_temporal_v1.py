...
(from train_temporal_v1.py's checkpoint output) against UCO-LAEO, with
NO training. The temporal equivalent of eval_static_v1.py.

UPDATED 2026-08-20: added paired per-record export (model error AND
static-copy-forward baseline error, per row) so a genuine paired
bootstrap CI can be computed on the UCO-LAEO zero-shot temporal result --
closing the specific, disclosed gap in Section 5.6/6.2 ("reports model
accuracy directly, not a baseline comparison... has not been subjected
to the same formal significance testing" as every in-domain temporal
claim). Same additive pattern as eval_static_v1.py's own per-record
export: existing return signature and existing caller behaviour are
completely unchanged when EVAL_TEMPORAL_OUTPUT_DIR / STATIC_CONDITION
are set as before -- this only adds a new saved file, nothing removed
or restructured.

The baseline's own prediction requires no new data: the static model's
predicted (x,y) at the LAST context-window frame -- already cached in
predicted_xy, already part of embed_seq's own concatenation (Section
3.5.1) -- IS the static-copy-forward baseline by definition (Section
3.5.5: "the static student's own most recent prediction, carried
forward unchanged"). No re-inference needed, just reading one more
already-cached value per row.
    ...
    with open(UCOLAEO_TEMPORAL_MANIFEST_PATH, encoding="utf-8") as f:
        for line in f:
            ...
            rows.append({
                ...
                # Static-copy-forward baseline prediction: the static model's
                # own predicted (x,y) at the LAST context frame, already
                # cached -- no re-inference needed. Kept separate from
                # embed_seq (which uses predicted_xy for EVERY window frame)
                # since this specific value is what the baseline itself is.
                "baseline_pred_x": float(predicted_xy[embed_indices[-1]][0]),
                "baseline_pred_y": float(predicted_xy[embed_indices[-1]][1]),
...
def evaluate_model(model, rows, dims):
    """UPDATED 2026-08-20: now also computes the static-copy-forward
    baseline's own error per row, paired with the model's error on the
    exact same record -- baseline_dists and model dists are guaranteed
    aligned by construction (built in the same loop, same row, same
    on-the-fly dims lookup), which is what a paired bootstrap requires.
    Existing (dists, by_gaze_type) return values are completely
    unchanged; the two new lists are additional, not replacements."""
    dists, by_gaze_type = [], {}
    baseline_dists = []
    per_record_gaze_type = []
    with torch.no_grad():
        for r in rows:
            ...
            target_x, target_y = r["target_gaze_x"] / w, r["target_gaze_y"] / h
            target = torch.tensor([[target_x, target_y]]).to(DEVICE)
            ...
            baseline_d = math.hypot(r["baseline_pred_x"] - target_x, r["baseline_pred_y"] - target_y)
            baseline_dists.append(baseline_d)
            per_record_gaze_type.append(r["target_gaze_type"])

    return dists, by_gaze_type, baseline_dists, per_record_gaze_type
...
def main():
    ...
    for arch in TEMPORAL_ARCHITECTURES:
        ...
        dists, by_gaze_type, baseline_dists, per_record_gaze_type = evaluate_model(model, rows, dims)
        pooled_ade = sum(dists) / len(dists)
        pooled_baseline_ade = sum(baseline_dists) / len(baseline_dists)
        by_gt_ade = {g: sum(v) / len(v) for g, v in by_gaze_type.items()}
        print(f"  {arch:<22} pooled_ADE={pooled_ade:.4f}  baseline_ADE={pooled_baseline_ade:.4f}  "
              f"n={len(dists)}  by_gaze_type={ {g: round(v, 4) for g, v in by_gt_ade.items()} }")
        all_results[arch] = {"pooled_ade": pooled_ade, "pooled_baseline_ade": pooled_baseline_ade,
                              "n": len(dists), "by_gaze_type": by_gt_ade}

        # Paired per-record export -- model error and baseline error on the
        # exact same records, same order -- enables a genuine paired
        # bootstrap CI (bootstrap_ci_static.py's own paired_bootstrap_ci()
        # works directly on these two lists, same as it does for the
        # static gt/mtgs/iv3/hybrid comparison).
        per_record_path = os.path.join(
            OUTPUT_DIR, f"per_record_ade_temporal_{STATIC_CONDITION}_{arch}_{TARGET_HORIZON_MS}ms.json")
        with open(per_record_path, "w") as f:
            json.dump({"model_ade": dists, "baseline_ade": baseline_dists,
                       "gaze_type": per_record_gaze_type}, f)
        print(f"  Saved paired per-record distances for bootstrap CI: {per_record_path}")
