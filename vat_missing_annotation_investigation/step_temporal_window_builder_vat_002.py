...
# ══════════════════════════════════════════════════════════════════════
# ── GAP BLOCK DIAGNOSTIC — contiguous runs, independent of windowing ────
# ══════════════════════════════════════════════════════════════════════

def compute_gap_blocks(qualifying_clips, output_path="vat_temporal_gap_blocks.csv"):
    """Reports TRUE contiguous missing-frame runs per (clip, subject),
    computed directly against the canonical timeline — independent of
    window slicing. This exists because scan_gaps()'s per-window report
    fragments a single real gap into many overlapping window-level
    entries (stride-4 windows re-report the same underlying block
    repeatedly); this function collapses that back down to what the
    gap actually IS: one or more contiguous spans where a subject has
    no annotation, each reported once.

    Also reports block position within the clip (start_pct/end_pct of
    the canonical timeline) — useful for distinguishing "subject not yet
    introduced / gaze not yet determinable at clip start" from "subject
    briefly undeterminable mid-clip" from "subject exits before clip end",
    which are different phenomena worth telling apart in the writeup
    (see 2026-07-06 Sound of Music / clip 0_270 finding: contiguous
    blocks corresponded to a sustained period where gaze direction
    genuinely wasn't determinable, not tracking noise)."""
    block_rows = []
    ...
    for clip in qualifying_clips:
        ...
        for subject, ann in clip["subjects"].items():
            missing_flags = [fname not in ann for fname in timeline]
            ...
            while i < n:
                ...
                block_rows.append({
                    ...
                    "clip": clip["clip"],
                    "subject": subject,
                    "block_start_fname": timeline[block_start],
                    "block_end_fname": timeline[block_end],
                    "block_length_frames": block_length,
                    "clip_total_frames": n,
                    "block_start_pct": round(block_start / n * 100, 1),
                    "block_end_pct": round(block_end / n * 100, 1),
                    "touches_clip_start": block_start == 0,
                    "touches_clip_end": block_end == n - 1,
                })

    with open(output_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=[
            "show", "clip", "subject", "block_start_fname", "block_end_fname",
            "block_length_frames", "clip_total_frames",
            "block_start_pct", "block_end_pct",
            "touches_clip_start", "touches_clip_end",
        ])
        writer.writeheader()
        writer.writerows(block_rows)

    n_blocks = len(block_rows)
    n_at_start = sum(1 for r in block_rows if r["touches_clip_start"])
    n_at_end = sum(1 for r in block_rows if r["touches_clip_end"])
    n_touching_either = sum(
        1 for r in block_rows if r["touches_clip_start"] or r["touches_clip_end"]
    )
    n_mid = n_blocks - n_touching_either
    avg_len = (sum(r["block_length_frames"] for r in block_rows) / n_blocks) if n_blocks else 0

    print("=" * 60)
    print("Gap block diagnostic (true contiguous runs, not per-window)")
    print("=" * 60)
    print(f"  Total distinct gap blocks     : {n_blocks}")
    print(f"  Blocks touching clip start    : {n_at_start}")
    print(f"  Blocks touching clip end      : {n_at_end}")
    print(f"  Blocks fully mid-clip         : {n_mid}")
    print(f"  Mean block length (frames)    : {avg_len:.1f}")
...
def main():
    ...
    compute_gap_blocks(qualifying_clips)
