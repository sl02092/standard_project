...
TEST_MODE = False   # TEST_MODE run already validated the pipeline mechanics
                     # end to end -- see MEDIUM_MODE below for the next step.
MEDIUM_MODE = True   # Deliberate stepping stone before an unrestricted full
                      # run: enough frames to test whether the TEST_MODE
                      # result (predictions clustering per-show rather than
                      # varying per-frame) resolves with more data, while
                      # keeping runtime bounded and predictable.
...
OUTPUT_DIR = ("lora_regression_test" if TEST_MODE
              else "lora_regression_medium" if MEDIUM_MODE
              else "lora_regression_run")
...
def load_split_manifest():
    ...
    if TEST_MODE:
        ...
        # Deliberate stepping stone before an unrestricted full run --
        # enough frames per show to test whether TEST_MODE's per-show
        # clustering pattern resolves with more data, while runtime stays
        # bounded and predictable (see docstring for the reasoning).
        train_df = train_df.groupby("show", group_keys=False).head(60).reset_index(drop=True)
        val_df = val_df.groupby("show", group_keys=False).head(20).reset_index(drop=True)
...
def evaluate(internvl_model, internvl_processor, reg_head, val_df, base_path, device, tag="eval"):
    ...
    # NEW: numeric check for the TEST_MODE finding (predictions clustering
    # tightly per-show rather than varying per-frame) -- doesn't scale to
    # eyeballing once val has more than ~10 rows, so quantify it directly.
    # Compares std of PREDICTIONS within each show against std of ACTUAL
    # ground truth within that same show. If pred_std << gt_std across
    # shows, that's the clustering pattern confirmed numerically, not
    # genuine per-frame differentiation.
    from collections import defaultdict
    by_show = defaultdict(list)
    for show, gx, gy, px, py, d in raw_predictions:
        by_show[show].append((gx, gy, px, py))

    print(f"  [{tag}] per-show prediction spread (pred_std should be "
          f"comparable to gt_std if the model is differentiating frames, "
          f"not just guessing a per-show average):")
    for show, vals in by_show.items():
        if len(vals) < 2:
            continue
        gt_xs = [v[0] for v in vals]
        gt_ys = [v[1] for v in vals]
        pred_xs = [v[2] for v in vals]
        pred_ys = [v[3] for v in vals]
        gt_std = (sum((x - sum(gt_xs)/len(gt_xs))**2 for x in gt_xs) / len(gt_xs)) ** 0.5
        pred_std = (sum((x - sum(pred_xs)/len(pred_xs))**2 for x in pred_xs) / len(pred_xs)) ** 0.5
        print(f"    {show:<20} n={len(vals):<3} gt_x_std={gt_std:.4f}  pred_x_std={pred_std:.4f}"
              f"  ratio={pred_std/gt_std if gt_std > 1e-6 else float('inf'):.2f}")
...
def main():
    ...
    mode_name = "TEST_MODE" if TEST_MODE else "MEDIUM_MODE" if MEDIUM_MODE else "FULL"
