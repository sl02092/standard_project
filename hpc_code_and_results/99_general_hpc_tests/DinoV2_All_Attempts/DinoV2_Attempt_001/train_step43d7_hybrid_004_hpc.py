...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            if (batch_idx + 1) % 200 == 0:
                ckpt_path = os.path.join(
                    ...
                    f"dino_internvl_projector_ckpt_epoch{epoch+1}_step{batch_idx+1}.pt"
                )
        ...
        epoch_ckpt_path = os.path.join(os.path.dirname(MANIFEST_PATH), f"dino_internvl_projector_ckpt_epoch{epoch+1}_final.pt")
        torch.save(proj_head.state_dict(), epoch_ckpt_path)
