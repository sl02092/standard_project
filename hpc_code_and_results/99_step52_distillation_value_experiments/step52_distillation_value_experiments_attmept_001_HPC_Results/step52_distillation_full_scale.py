"""
step52_distillation_full_scale.py - Distillation Value Experiment, FULL SCALE
Re-runs the interim-report question -- "does the student learn better from
teacher labels than from GT alone?" -- at real scale, on the current best
production teacher labels, rather than the original 85-frame test subset.

WHY THIS RUN EXISTS (2026-07-04): Objective 2's actual bar is downstream
STUDENT performance vs GT, not teacher ADE vs a fully-supervised specialist
(MTGS et al.) -- that comparison was never what Objective 2 asked, and
raw-ADE panic over the LoRA fine-tuning collapse was answering the wrong
question. This is the right question, run properly at scale instead of on
85 frames.

CHANGES FROM Step52_distillation_experiment.py:
  1. LABELS_FILE points at labels_hybrid_targeted.jsonl (current best/
     frozen production teacher: InternVL3 + GDINO targeted-phrase prompt),
     not the original 85-frame labels_test.jsonl.
  2. FIX: off-screen row filtering. The original script's record loader
     only checked `if pred_x/gt_x is None`. Tonight's OWN comparison
     script (ade_by_gaze_type_comparison.py) already found that off-screen
     rows in production label files have gt_x/gt_y populated with
     MIS-NORMALIZED GARBAGE (-1/img_width), not None or a clean sentinel --
     so the None-check silently let ~4,798 off-screen rows with nonsense
     coordinates into "on-screen" training/eval. Fixed here by filtering
     on gt_px_x/gt_px_y == -1 (raw pixel sentinel), the same check already
     established as correct tonight. Without this fix, this run would
     reproduce a bug already found and fixed once today, in a different
     script.
  3. NUM_RECORDS_CAP added (default: no cap, use everything) -- at full
     VAT scale (~18,298 on-screen frames) this is still a lightweight
     ViT-Tiny model with no per-sample VLM calls, so full-scale is
     tractable in one HPC job, unlike the teacher pipeline itself.
  4. OUTPUT_DIR changed to keep this run's artifacts separate from the
     original 85-frame experiment_results/.

EVERYTHING ELSE (architecture, training loop, train/val split-by-clip,
evaluation methodology, both models trained on identical held-out clips
and evaluated against the same GT) is UNCHANGED from the original,
already-validated Step52 design.
"""
...
OUTPUT_DIR   = "experiment_results_full_targeted"
