"""
step45_hybrid_teacher_pipeline.py — Hybrid Teacher Pipeline
InternVL3-8B-hf (semantic identification) + Grounding DINO (text-conditioned
spatial localization) as a combined teacher, replacing InternVL3-alone for
object-gaze coordinate prediction.

MOTIVATION (from experimental evidence, step44 investigation):
  - InternVL3-alone object-gaze ADE: 0.436 (production scale, 5,328 frames)
  - Hybrid approach (6-frame PoC): mean ADE 0.248-0.302 depending on prompt
  This pipeline tests whether that improvement holds at production scale,
  producing labels_hybrid.jsonl in identical schema to labels_full.jsonl
  for direct apples-to-apples comparison.

ROUTING LOGIC (same as step40, deliberately unchanged):
  ...
  - use_teacher == True, gaze_type == "object"  → HYBRID (InternVL3 + GDINO)
    (this is the bottleneck the hybrid is designed to fix)
  - GDINO no-box-found or InternVL3 off-screen on object frame → GT fallback

THREE MODES (same as step40):
...
OUTPUT: labels_hybrid.jsonl (same schema as labels_full.jsonl)
  teacher_version field distinguishes hybrid labels:
  "internvl3-8b-v4-prompt"       for social frames (InternVL3-alone, unchanged)
  "internvl3+gdino-base-v1"      for object frames (new hybrid)

CHANGELOG vs step45_hybrid_teacher_pipeline.py (never ran — queued, then pulled
before starting):
  - FIX: run_social_teacher() cast inputs to dtype=torch.float16 while the
    model is loaded in torch.bfloat16 (line ~403). This was almost certainly
    going to crash on the first social-frame inference call (GT-direct rows
    don't touch the model, so it would have died fast). Now bfloat16,
    matching run_hybrid_object() and the model load dtype.
  - TEST_MODE flipped True for this run — step45 logic has never executed
    end-to-end (GT + social + object routing, checkpointing/resume). Confirm
    clean before flipping back to False for the full 23,096-frame run.
"""
...
TEACHER_VERSION_HYBRID = "internvl3+gdino-base-v1"   # object frames: new hybrid

# Output files
if PROMPT_TEST:
    LABELS_FILE   = "labels_hybrid_prompt_test.jsonl"
    PROGRESS_FILE = "progress_hybrid_prompt_test.json"
    LOG_FILE      = "pipeline_hybrid_prompt_test.log"
elif TEST_MODE:
    LABELS_FILE   = "labels_hybrid_test.jsonl"
    PROGRESS_FILE = "progress_hybrid_test.json"
    LOG_FILE      = "pipeline_hybrid_test.log"
else:
    LABELS_FILE   = "labels_hybrid.jsonl"
    PROGRESS_FILE = "progress_hybrid.json"
    LOG_FILE      = "pipeline_hybrid.log"
...
# Best-performing identification prompt from step44 investigation
IDENTIFY_PROMPT = (
    "Analysis Steps:"
