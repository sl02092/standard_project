...
# block -- inference/routing/prompt logic below is completely untouched
# from the verified VAT production script.
#
# PROMPT_TEST/TEST_MODE now env-overridable rather than hand-edited between
# runs -- lets a calibration probe run without touching the file (same
# reasoning distillation_four_way_real.py's EPOCHS/CONDITIONS overrides
...
# Dead code upstream (BASE_PATH is never actually used to build a path --
# every img_path is read directly from the manifest row), kept only for
# parity with the VAT-production script this was copied from.
...
TEACHER_VERSION_SOCIAL = "internvl3-8b-v5-noleak"           # social frames unchanged
TEACHER_VERSION_HYBRID = "internvl3+gdino-base-v3-alwaysdino" # object frames: targeted-phrase prompt

# Output files -- ALL DISTINCT from VAT's own filenames (labels_hybrid*.jsonl
# etc.) by default, so this can never collide with the frozen VAT production
# output even if run from the same directory. Same env-override pattern as
# LABELS_HYBRID_PATH/DISTILL_OUTPUT_DIR in distillation_four_way_real.py.
_OUT_PREFIX = os.environ.get("TEACHER_OUTPUT_PREFIX", "childplay_hybrid_targeted")
if PROMPT_TEST:
...
# Prompt test / test mode settings -- ChildPlay's "show" = video_id (per
# labels_childplay_gt.jsonl's own established convention). Defaults are the
# four probe clips picked 2026-07-27 (diverse: 4 different channels, both
# resolution classes, 3 different fps, 3 different splits) -- see chat for
# selection rationale. Override via env var (comma-separated) for a
# different probe set without editing this file.
PROMPT_TEST_N     = 10
