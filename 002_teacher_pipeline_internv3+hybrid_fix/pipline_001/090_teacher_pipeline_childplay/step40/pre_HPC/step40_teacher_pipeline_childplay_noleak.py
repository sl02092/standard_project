...
# CHILDPLAY ADAPTATION (2026-07-27), all changes confined to this config
# block -- same pattern as the step46 ChildPlay copy, see that file's
...
# read directly from the manifest row), kept only for parity with VAT.
MANIFEST_PATH = os.environ.get(
    "TEACHER_MANIFEST_PATH",
    "/parallel_scratch/sl02092/standard_project/frame_manifest_childplay.csv",
)
...
# Output files -- distinct from VAT's own filenames (labels_full.jsonl etc.)
# by default, so this can never collide with frozen VAT production output.
_OUT_PREFIX = os.environ.get("TEACHER_OUTPUT_PREFIX", "childplay_full")
if PROMPT_TEST:
...
# ChildPlay's "show" = video_id (matches labels_childplay_gt.jsonl's own
# established convention). Defaults = the four probe clips picked
# 2026-07-27 (diverse: 4 channels, both resolutions, 3 fps values, 3
# splits) -- override via env var (comma-separated) without editing this file.
PROMPT_TEST_N = 10
_default_test_shows = "DGls-Z4e6OI,4yWavYq9_Ks,Kaf6jiHMmYc,1Ab4vLMMAbY"
...
def load_manifest():
    # dtype forced for "subject": ChildPlay's person_ids are bare digits
    # ("1","2","3"), unlike VAT's, which pandas would otherwise silently
    # infer as int64 -- breaks the pair_id string concat below AND, further
    # downstream, JSON serialization of "subject" in every output record.
