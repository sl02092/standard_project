...
# Locked schema version string — UNCHANGED from VAT/ChildPlay: same model,
# same prompt, just different data.
TEACHER_VERSION = "internvl3-8b-v5-noleak"

# Output files -- distinct from VAT's/ChildPlay's own filenames.
_OUT_PREFIX = os.environ.get("TEACHER_OUTPUT_PREFIX", "ucolaeo_full")
if PROMPT_TEST:
...
# UCO-LAEO's "show" = video_id (got01, mr08, sv03, twd03, ...). Probe set
# picked 2026-07-29: one from each content category (got/mr/sv/twd), all
# well-populated (372-904 rows each) -- deliberately none from
# 'negatives*' (those contribute zero rows by construction, would give an
# empty/meaningless probe).
PROMPT_TEST_N = 10
...
def load_manifest():
    # dtype forced for "subject": UCO-LAEO's person_ids are bare digits
    # ("1","2","3"...), same as ChildPlay's -- pandas would otherwise infer
    # int64, breaking the pair_id string concat + JSON serialization
    # downstream (confirmed bug class from the ChildPlay run, applied here
    # proactively rather than waiting to hit it again).
