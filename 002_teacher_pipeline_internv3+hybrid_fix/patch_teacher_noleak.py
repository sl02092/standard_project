...
# ── .sub files ────────────────────────────────────────────────────────
# ══════════════════════════════════════════════════════════════════════
PY_ON_LINE_RE = re.compile(r"^(.*python3\s+)(-u\s+)?(step4[06]_\S+?)\.py\s*$", re.MULTILINE)
JOBNAME_RE    = re.compile(r'^(#SBATCH\s+--job-name=)"?([^"\n]+)"?\s*$', re.MULTILINE)
HF_OFFLINE_RE = re.compile(r"^export HF_HUB_OFFLINE=1\s*$", re.MULTILINE)
...
def patch_sub(src, tag):
    changes = []

    hits = list(PY_ON_LINE_RE.finditer(src))
    if len(hits) != 1:
        raise PatchError(f"python3 <script>.py line: expected 1, found {len(hits)}")
    h = hits[0]
    stem = h.group(3)
    if stem.startswith("step40"):
        suffix, kind = "_noleak", "step40"
    else:
        suffix, kind = "_alwaysdino", "step46"

    # -u is not cosmetic: without it stdout is block-buffered, so the slurm
    # .out file stays empty for hours and you cannot see whether the job is
    # progressing or wedged.
    had_u = bool(h.group(2))
    out = src[:h.start()] + f"{h.group(1)}-u {stem}{suffix}.py" + src[h.end():]
    changes.append(f"runs {stem}{suffix}.py" + ("" if had_u else "  (+ added -u for live stdout)"))

    jn = list(JOBNAME_RE.finditer(out))
    if len(jn) == 1:
        j = jn[0]
        out = out[:j.start()] + f'{j.group(1)}"{j.group(2)}_{tag}"' + out[j.end():]
        changes.append(f'job-name -> {j.group(2)}_{tag}')

    hf = list(HF_OFFLINE_RE.finditer(out))
    if len(hf) != 1:
        raise PatchError(f"export HF_HUB_OFFLINE=1 anchor: expected 1, found {len(hf)}")
    at = hf[0].end()
    hybrid = ('export HYBRID_ALWAYS_DINO=1      # 0 restores the frozen 0.30/0.25 thresholds\n'
              if kind == "step46" else "")
    out = out[:at] + SUB_ENV_BLOCK.format(tag=tag, hybrid=hybrid) + out[at:]
    changes.append(f"TEACHER_RERUN_TAG={tag}" + ("  + HYBRID_ALWAYS_DINO=1" if hybrid else ""))

    out += (f"\n# NOTE: outputs are now labels_*_{tag}.jsonl / progress_*_{tag}.json /\n"
            f"# pipeline_*_{tag}.log -- any filename in the comments above is the\n"
            f"# FROZEN run's and is no longer what this job writes.\n")
    return out, changes
...
def main():
    ...
    for path in a.files:
        ...
        if base.endswith(".sub"):
            kind = "sub"
            # the .sub filename need not match the .py it launches, so the
            # suffix is decided from the script named inside it
            m = PY_ON_LINE_RE.search(src)
            suffix = "_noleak" if (m and m.group(3).startswith("step40")) else "_alwaysdino"
        elif base.startswith("step40"):
            kind, suffix = "step40", "_noleak"
        elif base.startswith("step46"):
            kind, suffix = "step46", "_alwaysdino"
        else:
            print(f"SKIP  {base}  (not a step40_/step46_/.sub file)")
            continue

        try:
            out, changes = {"step40": patch_step40,
                            "step46": patch_step46,
                            "sub":    patch_sub}[kind](src, a.tag)
            if kind != "sub":
        ...
        stem, ext = os.path.splitext(base)       # keep .sub as .sub, .py as .py
        dest = os.path.join(dest_dir, stem + suffix + ext)
