"""
Step 4 - Teacher Labelling Pipeline (v5 — definitive/canonical version)
Reads frame_manifest.csv and generates gaze labels for each frame.

KEY CHANGES FROM v4 (this file — v5) — 2026-07-19
───────────────────────────────────────────────────
Prompted by an external code review of v4. Two of its four claims were real,
confirmed bugs (fixed below); two were not (see reasoning inline, and do not
re-fix without re-checking against real data first):
  - NOT FIXED: "O(N²) get_other_subjects() / severe I/O starvation" — the
    get_other_subjects() scan is genuinely O(N) per call (real, and the
    groupby lookup below removes it), but the claimed "40s/frame" driving a
    "70-80% speedup from batching" was a misread: 40 is a hardcoded
    pre-run ETA placeholder (see SECS_PER_FRAME_ESTIMATE below), never a
    measurement. The actual completed v4 run (slurm_gpu-node02_623637)
    averaged ~1.14s/frame over 18,298 frames — VLM inference, not the
    dataframe scan, is almost certainly the real cost. Batching/async I/O
    was NOT implemented; no evidence it's needed.
  - FIX (real, low-impact this run): get_other_subjects() now uses a
    pre-built groupby lookup instead of re-scanning the full dataframe on
    every call. Correct either way; real run was already fast enough that
    this wasn't urgent, but it's free and worth having in the canonical
    version.
  - FIX (real bug): off-screen substitution. v4 detected the teacher
    hallucinating solo cases correctly, but when the teacher predicted
    off-screen (-1,-1) and disagreed with GT (GT says on-screen), v4
    silently wrote GT's real coordinate as if it were the teacher's own
    prediction, tagged "PARSED" — masking exactly the cases where the
    teacher is wrong, and leaking GT into the "teacher" condition on its
    own errors. Now writes -1.0,-1.0 (the teacher's actual claim) whenever
    the teacher says off-screen, regardless of whether GT agrees.
    label_source stays "teacher_offscreen" either way, so this is fully
    traceable downstream. In the completed v4 run this affected exactly 1
    of 23,096 records (labels_full.jsonl, verified directly) — negligible
    for existing results, but a real distillation-fidelity issue and cheap
    to fix before any future run (ChildPlay/VACATION may hallucinate
    off-screen at a different rate).
  - FIX (real bug, zero actual occurrences in the v4 run): image-load
    fallback in the GT-writing loop silently guessed 1280x720 on any
    failure, which would silently corrupt pixel normalisation for any
    non-standard-resolution clip. Now logs and skips the frame instead of
    guessing. Verified zero image-load failures occurred in the completed
    v4 run (slurm log has none), so this changes nothing about existing
    results — it only protects future runs.

KEY CHANGES FROM v3 (retained from v4)
──────────────────────────────────────
...
# Dead code upstream (never actually used to build a path -- img_path is
# read directly from the manifest row), kept only for parity.
MANIFEST_PATH = os.environ.get(
    "TEACHER_MANIFEST_PATH",
    "/parallel_scratch/sl02092/standard_project/frame_manifest_vacation.csv",
)
MODEL_NAME    = "OpenGVLab/InternVL3-8B-hf"

# Locked schema version string — UNCHANGED from VAT/ChildPlay/UCO-LAEO:
# same model, same prompt, just different data.
TEACHER_VERSION = "internvl3-8b-v5-noleak"

# Output files -- distinct from every other dataset's filenames.
_OUT_PREFIX = os.environ.get("TEACHER_OUTPUT_PREFIX", "vacation_full")
if PROMPT_TEST:
...
# VACATION's "show" = video_id (plain numeric string, e.g. "106"). Probe
# set: 106 and 170 specifically, since these are the only two videos
# directly verified against real data during manifest-building (exact
# gaze-resolution matches, NA-exclusion confirmed) -- picking untested
# IDs risks a probe that happens to land on a low-yield video.
PROMPT_TEST_N = 10
_default_test_shows = "106,170"
...
def load_manifest():
    # dtype forced for "show"/"clip"/"subject": VACATION's show/clip are
    # bare numeric video IDs ("106", "170"...), which pandas would
    # otherwise infer as int64 -- same bug class as ChildPlay's "subject"
    # column, just landing on a different field here. Confirmed directly
    # against the real manifest before this caused the same pair_id
    # concat crash. subject ("Person1" etc) infers correctly as str on
    # its own but forced too for consistency/defense.
    df = pd.read_csv(MANIFEST_PATH, dtype={"show": str, "clip": str, "subject": str})
...
def build_frame_lookup(df):
    """
    v5: pre-built once, before the main loop, instead of get_other_subjects()
    re-scanning the full dataframe with a boolean mask on every call (that
    was a genuine O(N) scan x N calls = O(N^2) over the whole run). This
    groupby is O(N) total, built once. Confirmed this wasn't the dominant
    cost in the completed v4 run (real throughput was ~1.14s/frame, far
    faster than the O(N^2) cost would suggest if it dominated) -- this is
    correctness/hygiene for the canonical version, not a fix for an
    observed slowdown.
    """
...
def run_prompt_test(model, processor, df):
    ...
    frame_lookup = build_frame_lookup(df)  # v5: pre-built lookup, see above
    results = []
...
def main():
    ...
    df       = load_manifest()
    # v6 FIX (2026-07-22): previously trusted progress.json's "completed"
    # list directly (load_progress()). progress.json is only checkpointed
    # periodically, while append_label() writes every frame immediately --
    # this run was killed by a hard SLURM time-limit (not a clean stop) at
    # 91% complete, exactly the abrupt-kill scenario this bug is most
    # dangerous in. recover_progress_from_jsonl() already existed as a
    # documented manual recovery tool but was never wired into the default
    # resume path -- it now is, same fix already proven in step46/step47.
    completed = recover_progress_from_jsonl()
    progress = load_progress()
    if completed:
        print(f"Resuming: {len(completed)} pairs already done (reconstructed from {LABELS_FILE})")
    ...
    for i, (_, row) in enumerate(gt_rows.iterrows()):
        # v5 FIX: previously defaulted to 1280x720 on any load failure and
        # kept going -- this would silently corrupt pixel normalisation
        # (gx/img_w) for any clip that isn't actually 1280x720. Zero such
        # failures occurred in the completed v4 run (verified against its
        # slurm log), so this changes nothing about existing results; it
        # only protects future runs (ChildPlay/VACATION may have different
        # native resolutions). Skip and log instead of guessing.
        try:
    ...
    # v5 NOTE: this is a rough pre-run PLACEHOLDER, not a measurement -- the
    # completed v4 run actually averaged ~1.14s/frame. Don't read this
    # number as a profiled bottleneck (an external review did, incorrectly).
    SECS_PER_FRAME_ESTIMATE = 40
    log(f"Rough estimate (unmeasured placeholder): ~"
        f"{len(teacher_rows)*SECS_PER_FRAME_ESTIMATE/3600:.1f} hrs "
        f"at {SECS_PER_FRAME_ESTIMATE}s/frame -- actual rate will "
        f"be reported live below once inference starts.")
    ...
    for i, (_, row) in enumerate(
        ...
        if is_offscreen:
            # v5 FIX: v4 wrote gt_x_norm/gt_y_norm here -- i.e. whenever the
            # teacher claimed off-screen, v4 silently substituted GT's real
            # coordinate as the "teacher" prediction. When the teacher is
            # RIGHT (GT genuinely off-screen too, gt_x_norm is already None)
            # this was harmless. When the teacher is WRONG (GT says
            # on-screen, gt_x_norm is a real coordinate) this masked the
            # teacher's actual error and leaked GT into the "teacher"
            # condition's own training target -- exactly the case
            # distillation-fidelity depends on NOT covering for. Now
            # records what the teacher actually claimed (-1.0, -1.0)
            # regardless of GT agreement; label_source stays
            # "teacher_offscreen" so this is fully traceable downstream.
            # Verified: affected exactly 1/23,096 records in the completed
            # v4 run, so this does not change any existing result -- it
            # only matters for future runs.
