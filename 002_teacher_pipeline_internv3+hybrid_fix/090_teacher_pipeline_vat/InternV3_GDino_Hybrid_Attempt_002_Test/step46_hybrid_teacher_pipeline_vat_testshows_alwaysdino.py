...
# UNCHANGED deliberately, both tags: identical model/prompt as the existing
# train-split labels, just against MTGS's real TEST_SHOWS -- same tag is
# what makes the eventual merge (old train rows + these test rows, one file
# per condition, split column added after) coherent.
...
# so this can never overwrite labels_hybrid_targeted.jsonl etc.
if PROMPT_TEST:
    LABELS_FILE   = "labels_hybrid_targeted_vat_testshows_prompt_test.jsonl"
    PROGRESS_FILE = "progress_hybrid_targeted_vat_testshows_prompt_test.json"
    LOG_FILE      = "pipeline_hybrid_targeted_vat_testshows_prompt_test.log"
elif TEST_MODE:
    LABELS_FILE   = "labels_hybrid_targeted_vat_testshows_test.jsonl"
    PROGRESS_FILE = "progress_hybrid_targeted_vat_testshows_test.json"
    LOG_FILE      = "pipeline_hybrid_targeted_vat_testshows_test.log"
else:
    LABELS_FILE   = "labels_hybrid_targeted_vat_testshows_full.jsonl"
    PROGRESS_FILE = "progress_hybrid_targeted_vat_testshows_full.json"
    LOG_FILE      = "pipeline_hybrid_targeted_vat_testshows_full.log"
...
# Prompt test / test mode settings -- real MTGS TEST_SHOWS names (the old
# list was TRAIN_SHOWS names, which don't exist in the new manifest).
PROMPT_TEST_N     = 10
PROMPT_TEST_SHOWS = ["Titanic", "Survivor", "West World", "Downton Abby"]
TEST_MAX_FRAMES_PER_SHOW = 17
TEST_SHOWS = ["Titanic", "Survivor", "West World"]
