...
MANIFEST_PATH = "frame_manifest_vat_testshows.csv"
...
# UNCHANGED deliberately: this run uses the identical model/prompt as the
# existing train-split labels, just against MTGS's real TEST_SHOWS -- keeping
# the same tag is what makes the eventual merge (old train rows + these test
# rows, one file per condition, split column added after) coherent.
TEACHER_VERSION = "internvl3-8b-v5-noleak"

# Output files -- distinct from the existing train-split run's filenames,
# so this can never overwrite labels_full.jsonl etc.
if PROMPT_TEST:
    LABELS_FILE   = "labels_vat_testshows_prompt_test.jsonl"
    PROGRESS_FILE = "progress_vat_testshows_prompt_test.json"
    LOG_FILE      = "pipeline_vat_testshows_prompt_test.log"
elif TEST_MODE:
    LABELS_FILE   = "labels_vat_testshows_test.jsonl"
    PROGRESS_FILE = "progress_vat_testshows_test.json"
    LOG_FILE      = "pipeline_vat_testshows_test.log"
else:
    LABELS_FILE   = "labels_vat_testshows_full.jsonl"
    PROGRESS_FILE = "progress_vat_testshows_full.json"
    LOG_FILE      = "pipeline_vat_testshows_full.log"
...
# Prompt test settings -- real MTGS TEST_SHOWS names (the old list was
# TRAIN_SHOWS names, which don't exist anywhere in the new manifest and
