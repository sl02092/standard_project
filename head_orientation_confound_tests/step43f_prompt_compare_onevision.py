"""
step43e2_prompt_compare_llava.py — LLaVA-OneVision-Qwen2-7B variant of the
prompt comparison harness. Same TARGET_FRAMES, same prompt variants, same
lookup logic — optimized for LLaVA's separate image/text token collation.

Requires transformers >= 4.45.0 for native LlavaOnevision support.
"""
...
# ── RUNTIME ENGINE — LLaVA-OneVision loading + inference ──────────────────
...
def main():
    ...
    for clip, frame_num, subject in TARGET_FRAMES:
        ...
        for variant_name, build_fn in PROMPT_VARIANTS.items():
            ...
            # LLaVA-OneVision requires the text format structure to have 
            # a separated type mapping configuration for content assembly
            messages = [
                {
                    "role": "user",
                    "content": [
            ...
            # Standard chat template processing without direct tokenization
            prompt = processor.apply_chat_template(messages, add_generation_prompt=True)
            
            # Pass text and visual inputs explicitly to build consolidated tensors
            inputs = processor(
                images=img_raw, 
                text=prompt, 
                return_tensors="pt"
            ...
            # Strip the structural prompt token block from the generated text
