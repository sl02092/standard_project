"""
train_step43d7_hybrid_003.py — Sandbox Manifold Anchoring Optimization Harness
...
Implements structural token bookends ("sandwich layout") to eliminate vision blindness, 
forcing the frozen language model decoder to actively attend to cross-architecture vision tokens.
Features terminal layer normalization and stabilized weight initialization.

Both massive backbones are completely frozen.
"""
...
NUM_TRAIN_SAMPLES = 150  # ← Scale this up (e.g., 500 or 1000) as footprint capability permits
BATCH_SIZE        = 2
LEARNING_RATE     = 2e-5  
EPOCHS            = 5
...
class GazeHybridDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not os.path.exists(img_path):
            raise FileNotFoundError(f"[-] Could not resolve image path locally.")
...
def main():
    ...
    if not os.path.exists(MANIFEST_PATH):
        raise FileNotFoundError(f"[-] Definitively missing frame_manifest.csv at: {MANIFEST_PATH}.")
    ...
    if len(full_df) > NUM_TRAIN_SAMPLES:
        ...
        train_df = full_df
        print(f"[*] Footprint configuration utilizes entire dataset ({len(train_df)} items).")
    ...
    # Pre-tokenize the structural bookends to save operations inside the loop
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            for i in range(len(batch["prompts"])):
                ...
                # Convert structural strings into target LLM embeddings
                ...
                # ── FORCED ATTENTION SANDWICH LAYOUT ─────────────────────────────
                combined_embeds = torch.cat([
                    start_embeds,          # [1, start_len, llm_dim]
                    projected_vis[i:i+1],  # [1, vis_len, llm_dim]
                    end_embeds,            # [1, end_len, llm_dim]
                    p_embeds,              # [1, p_len, llm_dim]
                    t_embeds               # [1, t_len, llm_dim]
                ], dim=1)
                
                # Dynamically calculate individual token spans for precision masking
                ...
                # Force loss masking: mask out structural tokens, vision patches, and the prompt text
                labels = torch.full((1, total_len), -100, dtype=torch.long, device=llm_model.device)
                # Assign actual targets to the tail-end sequence space
