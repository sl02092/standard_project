...
#prompt_expanded = "Note: Ensure the coords match your the description of your analysis text.  If your text identifies the object as off-screen then set target coordinate to (-1, -1). It is of greatest importance that the coords are correct. If you know the person is looking offscreen, return offscreen. if you know the person is looking at an object spend time determining the position of the object before providing coordinates. Don't just give mid-screen coords, think about where the object actually is, don't just guess"
#prompt_expanded = "if the target is in-image, otherwise, if the object or person is outside the image then provide the coordinates to (-1, -1) NOT (0.5, 0.5)."
PROMPT_VARIANTS = {
...
#    "V1_Direct_Expanded": lambda w, h, box, lbl, others: f"Look at the person who is {lbl} at bounding box {box}. Predict their normalized gaze target coordinate as (X, Y) between 0.0 and 1.0. "+prompt_expanded,
#    "V2_Gaze_Follow_Expanded": lambda w, h, box, lbl, others: f"Trace the line of sight vector from the eyes of {lbl}. Provide the coordinate (X, Y) where their gaze lands. "+prompt_expanded,
