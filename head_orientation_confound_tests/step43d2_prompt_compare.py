...
2. Determine what they are paying attention to. Are they looking at a specific object, an available person, or entirely out of the frame boundaries (off-screen)?
3. Write your step-by-step reasoning in 1-2 sentences.
4. Conclude your response on a clean line with the exact format:
GAZE_XY: (x, y)"""
    return prompt
...
def build_prompt_eye_vs_pose_cot(img_w, img_h, primary_box, primary_label, others_with_labels):
    ...
    prompt += """\nInstructions:
- Note that a person's head can point one direction while their eyes look somewhere else. Isolate the exact glance direction of their eyes.
- Briefly describe what object or direction their glance target points to.
- Output the target coordinate. If looking off-camera/off-screen, strictly output (-1, -1).
Format structure:
Reasoning: [Your description]
GAZE_XY: (x, y)"""
    return prompt
...
def parse_gaze_xy(text):
    ...
    if re.search(r"GAZE_XY:\s*\(-1\s*,\s*-1\)", text, re.IGNORECASE): return -1.0, -1.0
