...
def encode_image_with_dino(image: Image.Image, dino_processor, dino_model) -> torch.Tensor:
    ...
    NOTE: torch.no_grad() is used instead of torch.inference_mode() so that
    the returned tensor is a normal (non-inference) tensor. inference_mode()
    marks tensors as non-differentiable, which causes a RuntimeError when they
    subsequently pass through any nn.Module (e.g. the projection head) that
    autograd is still tracking.
    """
...
def run_inference(image: Image.Image, prompt_text: str,
    ...
    The entire function runs under torch.no_grad() so no gradient graph is
    built at any stage. proj_head, dino_model, and llm_model are all frozen
    for inference, so gradients are never needed.
    """
    with torch.no_grad():
        # 1. Visual features
        ...
        # Decode — output_ids starts from token 0 of the generation when
        # inputs_embeds is used (no prompt token ids are prepended).
        response = llm_tokenizer.decode(output_ids[0], skip_special_tokens=True).strip()

    # Strip the echoed prompt if the model includes it in its output
