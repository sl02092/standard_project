"""
train_step43d7_hybrid_002.py — Alignment Optimization Harness (v2 - Manifest Driven)
...
teacher pipeline logic. Completely removes local annotation walking routines.
...
# Since this script lives in \head_orientation_confound_tests, we look one level up for the manifest
...
NUM_TRAIN_SAMPLES = 150  # ← ADJUST THIS: Number of random frames to sample for scaling your alignment footprint
BATCH_SIZE        = 2
LEARNING_RATE     = 5e-4
EPOCHS            = 5
    ...
    def __getitem__(self, idx):
        ...
        if not os.path.exists(img_path):
            ...
            if os.path.exists(potential_path):
                ...
                # Fallback layout: base_path/images/{clip}/{fname} (in case some shows are flat)
                potential_path2 = os.path.join(self.base_path, "images", clip, fname)
                if os.path.exists(potential_path2):
                    img_path = potential_path2
                else:
                    # Last-ditch effort: Extract the relative portion after "images" if it's embedded in the cluster path
                    if "images" in row["img_path"]:
                        rel_suffix = row["img_path"].split("images")[-1].lstrip("/\\")
                        img_path = os.path.join(self.base_path, "images", rel_suffix)

        # Ensure the script doesn't crash on unresolvable files
        if not os.path.exists(img_path):
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            for i in range(len(batch["prompts"])):
                ...
                # 1. Run the base language model backbone to get hidden states (Fixed unassigned attention_mask bug)
                outputs = llm_model(
