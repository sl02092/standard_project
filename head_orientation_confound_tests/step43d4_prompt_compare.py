...
Note: Ensure the coords match your . If the model text identifies the object as off-screen use (OFF, OFF)
it is of greatest importance that the coords are correct if you know the person is looking offscreen, return offscreen
if you know the person is looking at an object spend time determining the position of the object before providing coordinates.
Don't just give mid-screen co-ords, think about where the object is"""
    return prompt
...
def build_prompt_dynamic_cot(img_w, img_h, primary_box, primary_label, others_with_labels):
    ...
    # REWRITTEN SECTION: Hyper-strict positional anchoring steps
    prompt += f"""\nAnalysis Steps (YOU MUST GENERATE ALL 5 LINES IN ORDER):
1. State the precise eye/pupil direction relative to the subject's head box, which is located at y-range ({na[1]} to {na[3]}).
2. Identify the target object or state if the gaze points entirely outside the image frame boundaries (off-screen).
3. Estimate the exact normalised (x, y) coordinates of that target object. Note: If they are looking down at cards in their hands, the coordinates must be significantly lower down the image (higher y-value, e.g., near 0.8 or 0.9) than their head box which is at y={na[1]}.
4. Write 'Is_Off_Screen: Yes' or 'Is_Off_Screen: No'.
5. Conclude with the exact format line: GAZE_XY: (x, y)

Note: Ensure the coords match your analysis text. If the model text identifies the object as off-screen use (OFF, OFF). It is of greatest importance that the coords are correct if you know the person is looking offscreen, return offscreen. If you know the person is looking at an object, spend time calculating the exact location of that object relative to the frame. Do not default to mid-screen or standard quadrant coordinates like (0.25, 0.25) or (0.5, 0.5)."""
    return prompt
'''
...
def build_prompt_dynamic_cot(img_w, img_h, primary_box, primary_label, others_with_labels):
    ...
    # Calculate the exact numerical midpoint of the head box to give the model a central anchor
    head_cx = round((ax1 + ax2) / 2 / img_w, 3)
    
    na1 = round(ax1/img_w, 3)
    na2 = round(ay1/img_h, 3)
    na3 = round(ax2/img_w, 3)
    na4 = round(ay2/img_h, 3)
    
    prompt = f"Gaze estimation task. Image: {img_w}x{img_h}px.\n"
    prompt += f"SUBJECT (predict gaze for): {primary_label}, head box (normalised) ({na1},{na2}) to ({na3},{na4})\n"
    prompt += f"Head Horizontal Center-line (X): {head_cx}\n"
    ...
    # REWRITTEN SECTION: Forcing X-axis alignment and sub-pixel fraction reasoning
    prompt += f"""\nAnalysis Steps (YOU MUST GENERATE ALL 5 LINES IN ORDER):
1. Identify if the eyes are looking down/up and left/right. If looking at cards held in their lap, determine if the cards are aligned with their left eye, right eye, or centered between their hands.
2. Identify the target object or state if the gaze points entirely outside the image frame boundaries (off-screen).
3. Calculate the exact coordinates. Use these strict spatial logic rules:
   - For Y: If they look down at their hands, the Y-coordinate must be near the bottom edge of the frame (between 0.85 and 0.95).
   - For X: Look at the Head Horizontal Center-line ({head_cx}). If they hold an object naturally in front of them with both hands, the object's X-coordinate will usually shift past the head's left edge toward the true visual center of their body mass (often pushing the X-coordinate closer to 0.45 - 0.50). Do not blindly copy the head box's left X boundary.
4. Write 'Is_Off_Screen: Yes' or 'Is_Off_Screen: No'.
5. Conclude with the exact format line: GAZE_XY: (x, y)

Note: If the text identifies the target as off-screen, use (OFF, OFF). Do not use standard rounded quadrant defaults like (0.35, 0.85) out of habit; look closely at the true visual center-point of the object in the frame."""
    return prompt
