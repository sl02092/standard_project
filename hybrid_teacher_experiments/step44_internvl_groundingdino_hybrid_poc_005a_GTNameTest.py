...
"3. Identify what object or person are they looking at? "
"3. Write 1-2 sentences of step-by-step reasoning."
""
"If looking at an object: Answer with ONLY the object's name (2-4 words maximum), no extra description."
"If looking at a person, answer 'person'."
"If looking off-screen, answer 'off-screen'."
)

DESCRIBE_PROMPT_002a = (
"Analysis Steps:"
"1. Examine the subject's face orientation and precise eye/pupil direction."
"2. Check if the vector of their eye gaze points at a person, object or entirely outside the visible image frame boundaries."
"3. Identify what object or person are they looking at? "
...
"If looking at an object: Answer with ONLY the object's name (2-4 words maximum), no extra description."
"If looking at a person, answer 'person'."
"If there are multiple objects or people answer with the general location e.g. top-left, bottom-right, etc"
"If looking off-screen, answer 'off-screen'."
"Answer as concisely as possible object name/person and position - nothing else"
)
...
"If looking at a person, answer 'person'."
"If looking off-screen, answer 'off-screen'."
"Conclude your response on the final line with the exact format: approx GAZE_XY: (x, y)" 
"If answer 'off-screen' then GAZE_XY must be written as (-1, -1)."
"IMPORTANT: Answer as concisely as possible object name/person and approx GAZE_XY - nothing else"
)
...
def extract_object_phrase(description_text):
    """
    With DESCRIBE_PROMPT now asking InternVL3 for a bare object name directly
    (not a full sentence), this mostly just needs light cleanup -- strip
    quotes/punctuation/trailing period, catch the off-screen case, and fall
    back to the raw text if something unexpected comes back (e.g. the model
    ignores the instruction and answers in a full sentence anyway).
    """
    ...
    # Safety net: if the model ignored the "ONLY the object's name" instruction
    # and answered in a full sentence anyway, fall back to the old pattern
    # match rather than sending an entire sentence to Grounding DINO.
