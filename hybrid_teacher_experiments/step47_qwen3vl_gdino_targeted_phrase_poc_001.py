...
TARGETED_PROMPT = (
    ...
    "(e.g. 'hand of man in green coat', 'outstretched arm of woman in red'). 6 words maximum."
    ...
    "(e.g. 'person in gray suit', 'woman on the right', 'man with beard'). 6 words maximum."
    "If looking at an object: Answer with ONLY the object name plus a brief location hint if "
    "multiple similar objects exist (e.g. 'newspaper', 'red coffee mug', 'plate on the left'). 6 words maximum."
    " "
    "make sure the location is correct! - in previous tests you'd sometime say lady on the left, when she was on the right"
    "take time to identify the object - sometimes the object a person is looking at will be in front of a another person or other object."
)
    ...
    with open(filepath, "r") as f:
        for line in f:
            ...
            if frame_idx != target_idx:
                continue
            fname_actual = tokens[0].zfill(10) + ".jpg"
            try:
                gaze_x = float(tokens[6])
                gaze_y = float(tokens[7])
            except (ValueError, IndexError):
                gaze_x, gaze_y = -1.0, -1.0
            return {"fname_actual": fname_actual, "gaze_x": gaze_x, "gaze_y": gaze_y}
    return None
